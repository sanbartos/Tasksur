...
router.get("/conversation", isAuthenticated, async (req, res, next) => {
    try {
        const user = req.user;
        ...
        const messages = await storage.getMessagesBetweenUsers(user1, user2);
        res.json(messages);
    }
    catch (error) {
...
router.post("/", isAuthenticated, validateBody(sendMessageSchema), async (req, res, next) => {
    try {
        const user = req.user;
        if (!user?.id) {
            return res.status(401).json({ message: ERROR_MESSAGES.unauthorized });
...
router.post("/task", isAuthenticated, validateBody(sendMessageSchema), async (req, res, next) => {
    try {
        const user = req.user;
        if (!user?.id) {
            return res.status(401).json({ message: ERROR_MESSAGES.unauthorized });
...
router.get("/task/:taskId", isAuthenticated, validateIdParam("taskId"), canAccessTaskMessages, async (req, res, next) => {
    try {
        const taskId = parseInt(req.params.taskId);
...
router.patch("/:id/read", isAuthenticated, validateIdParam("id"), async (req, res, next) => {
    try {
