...
router.post("/articles", isAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO ${TABLE_NAME} (title, excerpt, tag, date, image, href)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [title, excerpt, tag, date, image, href]);
...
router.put("/articles/:id", isAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE ${TABLE_NAME} SET title=$1, excerpt=$2, tag=$3, date=$4, image=$5, href=$6 WHERE id=$7 RETURNING *`, [title, excerpt, tag, date, image, href, id]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: "Artículo no encontrado" });
        }
        res.json(result.rows[0]);
