...
export function registerCategoryRoutes(app) {
    ...
    app.post("/api/categories", isAuthenticated, authorizeRoles("admin"), validateBody(categorySchema), async (req, res, next) => {
    ...
    app.patch("/api/categories/:id", isAuthenticated, authorizeRoles("admin"), validateBody(categorySchema), async (req, res, next) => {
        const { id } = req.params;
        let { name, slug, description, icon, color } = req.body;
        if (!id) {
