 ...
 */
export function mapUserToDb(input) {
    const out = {};
    if (input.email !== undefined)
        out.email = input.email;
    if (input.firstName !== undefined)
        out.firstName = input.firstName;
    if (input.lastName !== undefined)
        out.lastName = input.lastName;
    if (input.profileImageUrl !== undefined)
        out.profileImageUrl = input.profileImageUrl;
    if (input.phone !== undefined)
        out.phone = input.phone;
    if (input.isTasker !== undefined)
        out.isTasker = input.isTasker;
    if (input.skills !== undefined)
        out.skills = JSON.stringify(input.skills);
    if (input.hourlyRate !== undefined)
        out.hourlyRate = input.hourlyRate == null ? null : String(input.hourlyRate);
    if (input.totalEarnings !== undefined)
        out.totalEarnings = input.totalEarnings == null ? null : String(input.totalEarnings);
    if (input.totalTasks !== undefined)
        out.totalTasks = input.totalTasks == null ? null : input.totalTasks;
    if (input.rating !== undefined)
        out.rating = input.rating == null ? null : String(input.rating);
    if (input.reviewCount !== undefined)
        out.reviewCount = input.reviewCount == null ? null : input.reviewCount;
    return out;
}
/* ---------- helpers ---------- */
...
function parseSkills(skills) {
    ...
    if (Array.isArray(skills))
        return skills.map(String).map(s => s.trim()).filter(Boolean);
