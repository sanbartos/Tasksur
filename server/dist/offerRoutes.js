...
export function registerOfferRoutes(app) {
    ...
    app.post('/api/offers', isAuthenticated, authorizeRoles('tasker'), async (req, res) => {
        try {
            const user = req.user;
            const userId = user.id;
    ...
    app.get('/api/tasks/:id/offers', async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.patch('/api/offers/:id', isAuthenticated, canModifyOffer, async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (status === 'accepted') {
                ...
                if (offer) {
                    ...
                    if (tasker && tasker.email) {
                        await sendEmail(tasker.email, 'Tu oferta ha sido aceptada', `Hola ${tasker.firstName ?? ''}, tu oferta para la tarea ha sido aceptada. ¡Felicidades!`);
                    }
                }
            }
