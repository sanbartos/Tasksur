...
import { isAuthenticated, AuthenticatedRequest } from "./middlewares/authMiddleware.js";
...
import type { User } from "./types/user.js"; // Ajusta la ruta
import { validateBody } from './middlewares/validateBody.js';
import { loginSchema, registerSchema } from './middlewares/validations.js'; // Ajusta ruta
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Registro de usuario
app.post('/api/register', validateBody(registerSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.post("/api/auth/login", validateBody(loginSchema), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    app.patch('/api/users/profile', isAuthenticated, async (req: Request, res: Response, next: NextFunction) => {
  const authReq = req as AuthenticatedRequest;
  const user = authReq.user as User | undefined;
...
app.get('/api/categories/:slug/tasks', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (queryFilters.categoryId) filters.categoryId = parseInt(queryFilters.categoryId as string);
    if (queryFilters.categoryName) filters.categoryName = queryFilters.categoryName as string;
    if (queryFilters.location) filters.location = queryFilters.location as string;
    if (queryFilters.status) filters.status = queryFilters.status as string;
    if (queryFilters.clientId) filters.clientId = queryFilters.clientId as string;
    if (queryFilters.taskerId) filters.taskerId = queryFilters.taskerId as string;
  ...
  async function canModifyTask(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  async function canModifyOffer(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    const userId = req.user?.id;
    const userRole = req.user?.role;
  ...
  app.get('/api/tasks/:id/offers', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const { page, limit } = req.query;
      const { pageNumber, limitNumber } = parsePaginationParams(page as string, limit as string);
...
app.get('/api/users/:id', async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (offer) {
          ...
          if (tasker && tasker.email) {
            ...
            await sendEmail(
              ...
              `Hola ${taskerName}, tu oferta para la tarea ha sido aceptada. ¡Felicidades!`
            );
          }
        }
      }
  ...
  app.get('/api/users/:id/reviews', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const { page, limit } = req.query;
      const { pageNumber, limitNumber } = parsePaginationParams(page as string, limit as string);
  ...
  async function canAccessTaskMessages(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    const userId = req.user?.id;
    const taskId = parseInt(req.params.id);
...
app.post('/api/payments', isAuthenticated, async (req: Request, res: Response, next: NextFunction) => {
  const authReq = req as AuthenticatedRequest;
  try {
    const userId = authReq.user?.id;
    if (!userId) {
  ...
  app.patch('/api/payments/:orderId/status', async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  app.get('/api/payments', isAuthenticated, async (req: Request, res: Response, next: NextFunction) => {
  const authReq = req as AuthenticatedRequest;
  try {
    const userId = authReq.user?.id;
    if (!userId) {
...
app.get('/api/debug/categories', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const categories = await storage.getCategories();
    res.json({
...
app.get("/api/debug/tasks-by-category", async (req: Request, res: Response) => {
  try {
    const prisma = (storage as any).prisma as import("@prisma/client").PrismaClient;
    ...
    const rows = rowsRaw as Array<{ slug: string; task_count: bigint }>;
