...
async function canModifyReview(req: Request, res: Response, next: NextFunction) {
  try {
    const user = req.user as User | undefined;
    if (!user) return res.status(401).json({ message: "No autorizado" });

    const reviewId = req.params.id;
    if (!reviewId) return res.status(400).json({ message: "ID inválido" });
...
export function registerReviewRoutes(app: Express) {
  ...
  app.get("/api/tasks/:id/reviews", async (req: Request, res: Response, next: NextFunction) => {
    try {
