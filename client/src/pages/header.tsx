...
import { Menu, Search, Plus, User, Settings, LogOut, Home, Bell } from "lucide-react";
...
export default function Header() {
  ...
  console.log("Notificaciones en Header:", notifications);
  ...
  const handleNotificationClick = (notificationId: number, messageId?: number) => {
    markAsRead(notificationId);
    setNotificationsOpen(false);
    
    // Redirigir según el tipo de notificación
    if (messageId) {
      setLocation(`/chat/${messageId}`);
    }
  };
                    ...
                    <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-md shadow-lg z-50 max-h-96 overflow-y-auto">
                      ...
                      {notificationsLoading ? (
                        ...
                        notifications.map((notification) => (
                          <div
                            key={notification.id}
                            onClick={() => handleNotificationClick(notification.id, notification.messageId ?? undefined)}
